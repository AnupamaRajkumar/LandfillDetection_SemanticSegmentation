# -*- coding: utf-8 -*-
"""LandfillPipeline_ResNet_UNet_ImageNet.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1wqtqK8PxGC4xLMOMPH0QWo5Ym-OwkXfR
...
class UNet(nn.Module):
  def __init__(self, pretrained=pretrained, enc_model=enc_model, requires_grad=True, remove_fc=remove_fc, num_classes=num_classes):
    ...
    self.dec5 = DecoderBlock(256+256, 256)

    self.up_conv4 = nn.ConvTranspose2d(256, 128,    
                                       kernel_size=3, stride=2, padding=1, 
                                       dilation=1, output_padding=1)
    self.dec4 = DecoderBlock(128+128, 128)              

    self.up_conv3 = nn.ConvTranspose2d(128, 64, 
                                       kernel_size=3, stride=2, padding=1, 
                                       dilation=1, output_padding=1)
    self.dec3 = DecoderBlock(64+64, 64)

    self.up_conv1 = nn.ConvTranspose2d(64, 32, 
                                       kernel_size=3, stride=2, padding=1, 
                                       dilation=1, output_padding=1)
    self.dec1 = DecoderBlock(32+64, 64)

    self.up_conv0 = nn.ConvTranspose2d(64, 32, 
                                       kernel_size=3, stride=2, padding=1, 
                                       dilation=1, output_padding=1)
    #self.dec1 = DecoderBlock(32+64, 64)
    #final layer
  ...
  def forward(self, x):
    x1, x2, x3, x4, x5, x6 = self.encoder_output(x)
    """
    print("x6:",x6.shape) 
    print("x5:",x5.shape)    
    print("x4:",x4.shape) 
    ...
    #print("dec1:",score.shape)
    #final layer
    score = self.final(self.up_conv0(score))
    #print(score.shape)
    return score
...
def modelPath(enc_model=enc_model):
  if(enc_model == 'resnet18'):
    model_path = './ResNet18_UNet.pth'
  if(enc_model == 'resnet34'):
    model_path = './ResNet34_UNet.pth'
  if(enc_model == 'resnet50'):
    model_path = './ResNet50_UNet.pth'
  if(enc_model == 'resnet101'):
    model_path = './ResNet101_UNet.pth'
  if(enc_model == 'resnet152'):
    model_path = './ResNet152_UNet.pth'
  if(enc_model == 'SolarisPreTrained'):
    model_path = './Solaris_resnet34_UNet.pth'
