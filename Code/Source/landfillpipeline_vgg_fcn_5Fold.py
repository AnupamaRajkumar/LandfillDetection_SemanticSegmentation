...
def TrainFunction():
  k_count = 0
  for train_index, valid_index in kf.split(dataFrame):
    train = dataFrame.iloc[train_index]
    valid = dataFrame.iloc[valid_index]
    train_dataset = CustomLandfillDataset(data=train_idx.values.tolist(), transforms=None)                               
    test_dataset = CustomLandfillDataset(data=test_idx.values.tolist(), transforms=None)   
    #data loader
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset,
    ...
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset,
                                              ...
                                              shuffle=True)
    k_count = k_count + 1
    print('----------Fold: {}/{}------------ '.format(k_count, n_splits,))


    val_loss_min = np.Inf
    print(len(train_losses), "\t", len(val_losses), "\t", len(accuracy), "\t", len(pixelAccs_mean))
    ...
    #fold_train_accuracy.append(accuracy)
    #fold_val_accuracy.append(pixelAccs_mean)
    val_losses_min.append(val_loss_min)

TrainFunction()

def PlotLoss(fold_train_loss, fold_val_loss, id):
  print('****Loss Plot {}********'.format(id))
  plt.plot(fold_train_loss, label='Training loss')
  plt.plot(fold_val_loss, label = 'Validation loss')
  plt.legend()

def PlotAccuracy(fold_train_accuracy, fold_val_accuracy, id):
  print('*******Accuracy Plot {}**********'.format(id))
  plt.plot(fold_train_accuracy, label='Training pixel accuracy')
  plt.plot(fold_val_accuracy, label='Validation pixel accuracy')
  plt.legend()

print("*******************Lowest validation losses are*******************")
print(val_losses_min)

PlotLoss(train_losses, val_losses, 5)

PlotAccuracy(accuracy, pixelAccs_mean, 5)
